  ...
  void _confirmArchive(BuildContext context, WidgetRef ref, Tour tour) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Are you sure you want to archive this tour? It will no longer appear on the traveler app.',
        ),
        actions: [
          TextButton(
            ...
            toursAsync.when(
              data: (tours) {
                ...
                final filteredTours = tours.where((tour) {
                  ...
                  final matchesSearch =
                      tour.title.toLowerCase().contains(q) ||
                  ...
                  // Status Filter
                  if (_selectedStatus == 'Active' && !tour.isActive) {
                    return false;
                  }
                  if (_selectedStatus == 'Archived' && tour.isActive) {
                    return false;
                  }
...
class _PremiumTourCardState extends ConsumerState<PremiumTourCard> {
  ...
  Widget build(BuildContext context) {
    final hasImage =
        widget.tour.heroImageUrl.isNotEmpty &&
        widget.tour.heroImageUrl.startsWith('http');
    final title = widget.tour.title.isEmpty
        ? 'Untitled Tour'
        : widget.tour.title;
    final destination = widget.tour.destination.isEmpty
        ? 'Unknown Destination'
                ...
                child: Stack(
                  ...
                  children: [
                    ...
                    Positioned(
                      ...
                      right: 16,
                      child: _GlassBadge(
                        text: widget.tour.category.isEmpty
                            ? 'Misc'
...
class _TourDataSource extends DataTableSource {
  ...
  DataRow? getRow(int index) {
    ...
    final hasImage =
        tour.heroImageUrl.isNotEmpty && tour.heroImageUrl.startsWith('http');
      ...
      cells: [
        ...
        DataCell(
          Container(
            ...
            child: Text(
              tour.category.isEmpty ? 'Misc' : tour.category,
              style: TextStyle(
                color: Theme.of(context).colorScheme.onSurfaceVariant,
                fontWeight: FontWeight.w500,
              ),
        ...
        DataCell(
          Text(
            '\$${tour.pricePerPerson.toStringAsFixed(2)}',
